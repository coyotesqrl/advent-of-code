...
;; [Link to code](https://github.com/coyotesqrl/advent-of-code/blob/main/src/coyotesqrl/2022/day07.clj)

(ns coyotesqrl.2022.day07
  (:require
...
(def input (->> "coyotesqrl/2022/day7-input.txt"
                (utils/input->seq)))

^{::clerk/visibility {:result :hide}}
(defn- dirtree [input]
  (letfn [(file-line [m s]
            (let [[_ len file-name] (re-find #"(\d*) (.*)" s)]
              (conj m [file-name (parse-long len)])))]
    (reduce (fn [a v]
              (cond
                (str/starts-with? v "dir") a
                (= v "$ ls") a
                (= v "$ cd /") (assoc a :cd ["/"])
                (= v "$ cd ..") (update a :cd #(vec (drop-last %)))
                (str/starts-with? v "$") (update a :cd conj (last (re-find #"\$ cd (.*)" v)))
                :else (update-in a (into (into [:tree] (:cd a)) [:files]) file-line v)))
            {:cd [] :tree {}}
            input)))

^{::clerk/visibility {:result :hide}}
(defn- subtree-size [tree]
  (reduce-kv (fn [a k v]
               (if (= :files k)
                 (+ a (apply + (map second v)))
                 (+ a (subtree-size v))))
             0
             tree))
...
(defn- dir-sizes
  [tree]
  (reduce-kv (fn [a k v]
               (let [sub-sizes (subtree-size v)]
                 (-> a
                     (merge (dir-sizes (dissoc v :files)))
                     (assoc [(random-uuid) k] sub-sizes))))
             {}
             tree))

^{::clerk/visibility {:result :hide}}
(defn- dir-size-report [input]
  (->> input
       (dirtree)
       :tree
       (dir-sizes)))
...
^{::clerk/visibility {:result :hide}}
(defn- used-space [input]
  (->> input
       (dir-size-report)
       (filter (fn [[k _]] (= "/" (second k))))
       (map val)
       (first)))

(let [used-space (used-space input)
      available-space (- 70000000 used-space)
      needed 30000000
      req-space (- needed available-space)]
  (->> input
       (dir-size-report)
       (map val)
       (filter #(<= req-space %))
