...
;; [Link to code](https://github.com/coyotesqrl/advent-of-code/blob/main/src/coyotesqrl/2022/day04.clj)

(ns coyotesqrl.2022.day04
  (:require
...
;; Elf in the pair would be exclusively cleaning sections their partner will already be cleaning, so these seem like the
;; most in need of reconsideration. In this example, there are `2` such pairs.
;;
;;**In how many assignment pairs does one range fully contain the other?**

#_(defn- gen-range [pair]
    (letfn [(to-range [[x y]]
              (range (parse-long x) (inc (parse-long y))))]
      (->> pair
           (map #(str/split % #"-"))
           (map to-range)))
    #_(let [[x y] (str/split s #"-")]
        (range (parse-long x) (inc (parse-long y)))))
