...
;; finding the errors. Every item type is identified by a single lowercase or uppercase letter (that is, `a` and `A`
...
(def rucksacks (->> "coyotesqrl/2022/day3-input.txt"
                    (utils/input->seq)
                    (map #(partition (/ (count %) 2) %))))

^{::clerk/visibility {:result :hide}}
(defn- common-element [sacks]
  (apply set/intersection (map set sacks)))
...
;; efficiency, within each group of three Elves, the badge is the **only item type carried by all three Elves**. That
...
^{::clerk/visibility {:result :hide}}
(def elf-groups (->> "coyotesqrl/2022/day3-input.txt"
                     (utils/input->seq)
                     (partition 3)))
